...
setwd("/Users/amulyadornala/Documents/Masters/Sem2/OR 568/Project/Dornala_Govindaswamy_Jain_Kothari_Kumaran_Veluri_FireRiskPrediction")
############ Address point dataset

address_points <- read.csv('Address_Points.csv', sep = ',')
...
finc <- read.csv('fire_incidents.csv', sep = ',')
...
finsp <- read.csv('fire_inspections.csv', sep = ',')
...
tax<- read.csv('Tax_Administration_s_Real_Estate_-_Dwelling_Data.csv', sep = ',')
...
tax_adm_real_estate <- read.csv('Tax_Administration_s_Real_Estate_-_Land_Data.csv', sep = ',')
...
confusionMatrix(Test$Incident_Happ,predictionCV)

accuracy2 <- (sum(diag(cm))/sum(cm))
accuracy2


predictionCV<-as.ordered(predictionCV)

confusionMatrix(Test$Incident_Happ,predictionCV)

dt_Roc <- roc(response = Test$Incident_Happ ,
              predictor = predictionCV)
...
prediction_rf <- predict(rf_classifier, newdata = Test[-c(9)])
confusionMatrix(Test$Incident_Happ,prediction_rf)

prediction_rf
prediction_rf<-as.ordered(prediction_rf)

rf_Roc <- roc(response = Test$Incident_Happ ,
              predictor = prediction_rf )
...
levels(clean_data$Incident_Happ) <- make.names(levels(names))
...
Train_$Incident_Happ<- as.factor(Train_$Incident_Happ)
set.seed(476)
gbmFit <- train(Incident_Happ ~ ., 
                data = Train_,
                method = "gbm",
                tuneGrid = gbmGrid,
                #metric = "ROC",
...
prediction_boost <- predict(gbmFit, newdata = Test[-c(9)])
confusionMatrix(Test$Incident_Happ,prediction_boost)

prediction_boost
prediction_boost<-as.ordered(prediction_boost)

boost_Roc <- roc(response = Test$Incident_Happ ,
              predictor = prediction_boost )
...
prob_gbm = predict(gbmFit, Test[-c(9)])

table(Test$HEAT_DESC)

Test1 <- Test %>% filter(HEAT_DESC != "Non Central") 
prob_gbm = predict(gbmFit, Test1[-c(9)])
prob_gbm

cm_test= confusionMatrix( data=prob_gbm, reference=Test1$Incident_Happ)
cm_test
